# --- validador_app.py ---
# Versión Atlantia 1.8 para Streamlit (Reintroduce V Umbrales como V11, ajusta V5.3 Geo)
...
atlantia_css = """
<style>
    /* ... (pega aquí TODO el CSS de la versión anterior 1.7) ... */
...
st.markdown("""
1.  **Selecciona el país** para el cual se aplicarán las reglas geográficas y de volumetría.
...
# [NUEVO] Umbrales numéricos (Volumetría)
THRESHOLDS_POR_PAIS = {
...
if uploaded_file_num is not None and uploaded_file_txt is not None:
    # ... (carga, optimización, V1-V8 igual que antes) ...
    ...
    num_cols = num_cols_base + [c for c in num_cols_extra if c in df_numerico_full.columns]
    num_ex = [c for c in num_cols if c in df_numerico_full.columns]; txt_ex = [c for c in txt_cols if c in df_textual_full.columns]
    try:
        df_numerico = df_numerico_full[[c for c in num_cols_base if c in df_numerico_full.columns]]
        df_textual = df_textual_full[[c for c in txt_cols if c in df_textual_full.columns]]
    except KeyError as e: st.error(f"Columna base esencial {e} no encontrada."); st.stop()

    # --- VALIDACIONES (V1-V11) ---

    # V1-V8 (Pega aquí el código de V1 a V8 de la versión 1.7)
    # V1: Tamaño
    ...
    try:
        ...
        # [MODIFICADO] Si clasif es None o está vacío {}, marca como Info
        if not clasif:
    ...
    # Ajuste: El estado general V5 solo falla si 5.3 falla (no si es Info)
    if status_v5 == "Correcto" and status_v5_3 not in ["Correcto", "Info"]: status_v5 = status_v5_3
    elif status_v5_3 == "Error": status_v5 = "Error" # Error en 5.3 siempre es Error general
    ...
    try:
        ...
        if np.isclose(suma_ponderador, total_filas, atol=1e-5): # Tolerancia absoluta pequeña
            content_v9 += "<span class='status-correcto-inline'>[Correcto]</span> Coinciden."
        else:
            status_v9 = "Incorrecto"; content_v9 += "<span class='status-incorrecto-inline'>[Incorrecto]</span> NO coinciden."
    ...
    errores_umbrales = []
    reglas_pais = THRESHOLDS_POR_PAIS.get(pais_clave_interna, []) # Reusa la clave interna
    if not reglas_pais:
        ...
        for regla in reglas_pais:
            ...
            except Exception as e: errores_umbrales.append({'Columna': col, 'Error': f'No numérico ({e})', 'ID': '-', 'Valor': '-'}); if status_v11 != "Error": status_v11 = "Error"; continue
            ...
            else: errores_umbrales.append({'Columna': col, 'Error': f'Cond "{cond}" no reconocida', 'ID': '-', 'Valor': '-'}); if status_v11 != "Error": status_v11 = "Error"; continue
            ...
            if not df_violaciones.empty:
                ...
                for idx, row in df_violaciones.iterrows():
                    ...
                    try: valor_violador_str = f"{valor_violador_num:,.2f}" if isinstance(valor_violador_num, (float, np.floating)) else f"{int(valor_violador_num):,}" # Formato con comas
